...
test('Marketplace migration is an explicit release command, not runtime startup', async () => {
  ...
  assert.match(packageJson.scripts.check, /validate-rehearsal-package\.mjs/)
...
test('zero-provision staging rehearsal pack is complete and cannot imply activation', async () => {
  const guide = await source('github-app/rehearsal/ZERO_PROVISION_REHEARSAL.md')
  const envTemplate = await source('github-app/rehearsal/staging.env.example')
  const settings = await source('github-app/rehearsal/github-app-settings.staging.json.example')
  const rollback = await source('github-app/rehearsal/ROLLBACK_DRILL.md')
  const receipts = await source('github-app/rehearsal/RCP_STAGING_RECEIPTS.md')
  const validator = await source('github-app/rehearsal/validate-rehearsal-package.mjs')

  assert.match(guide, /NO PROVIDER PROVISIONING/)
  assert.match(guide, /NO EXTERNAL CONFIGURATION MUTATION/)
  assert.match(guide, /NO SPEND/)
  assert.match(guide, /not evidence that staging is `PROVISIONED`, `CONFIGURED`, or `LIVE`/)
  ...
  assert.match(settings, /https:\/\/STAGING_HOST\/auth\/github\/callback/)
  assert.match(settings, /https:\/\/STAGING_HOST\/webhooks\/github/)
  assert.match(rollback, /fresh restore target/i)
  assert.match(rollback, /prior immutable OCI image/i)
  assert.match(receipts, /RCP-STG-001/)
  assert.match(receipts, /RCP-STG-010/)
  assert.match(validator, /Zero-provision staging rehearsal package validated/)
})

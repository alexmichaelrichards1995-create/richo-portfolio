...
This gate passed on PayCore Recovery CI #38 after the portable PostgreSQL transport was added. The signed fixture was an expired Checkout event with no intent metadata, so the expected invariant is exactly one processed webhook receipt and zero fabricated payment intents.

...
select ok(
  (select c.relrowsecurity from pg_class c join pg_namespace n on n.oid = c.relnamespace where n.nspname = 'public' and c.relname = 'payment_events'),
...
select policies_are(
  ...
  'payment_events',
  array[]::text[],
  'payment_events intentionally has no client policy'
);
...
-- service-role-controlled writes, and internal-only audit/payment events.
...
select ok(not has_table_privilege('authenticated', 'public.payment_events', 'SELECT'), 'authenticated cannot read payment event ledger');
select ok(has_table_privilege('service_role', 'public.payment_events', 'INSERT'), 'service_role can insert payment events');

...
import { listSecurityEvents, recordSecurityEvent } from "../lib/security-events.server";
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const [qualification, sessions, receipts, securityEvents] = await Promise.all([
    ...
    listSecurityEvents(session.shop, 40),
  ]);
  return { qualification, sessions, receipts, securityEvents, currentSessionId: session.id };
}
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (intent === "save_operator") {
    ...
    await upsertOperator({ shopDomain: session.shop, sessionId: targetSessionId, ...permissions });
    await recordSecurityEvent({ shopDomain: session.shop, event: "OPERATOR_CHANGED", actorId: session.id, targetId: targetSessionId, severity: "warn", payload: permissions });
    return { ok: true, intent };
  }

  if (intent === "revoke_sessions") {
    await recordSecurityEvent({ shopDomain: session.shop, event: "SESSION_REVOCATION_REQUESTED", actorId: session.id, severity: "warn" });
  ...
  if (intent === "set_deployment_approval") {
    ...
    await recordSecurityEvent({ shopDomain: session.shop, event: approved ? "DEPLOYMENT_APPROVED" : "DEPLOYMENT_APPROVAL_REMOVED", actorId: session.id, severity: approved ? "warn" : "info" });
    return { ok: true, intent };
  }
...
export default function SecurityControl() {
  const { qualification, sessions, receipts, securityEvents, currentSessionId } = useLoaderData<typeof loader>();
  return <main style={{ maxWidth: 1100, margin: "0 auto", padding: 24, fontFamily: "system-ui" }}>
    ...
    <section style={{marginTop:28}}><h2>Security Event Ledger</h2>{securityEvents.length===0?<p>No operational security events recorded.</p>:securityEvents.map(e=><article key={e.id.toString()} style={{borderBottom:"1px solid #eee",padding:"8px 0"}}><strong>{e.event}</strong><div><small>{e.createdAt.toISOString()} · {e.severity}{e.actorId ? ` · actor ${e.actorId}` : ""}{e.targetId ? ` · target ${e.targetId}` : ""}</small></div></article>)}</section>
    <section style={{marginTop:28}}><h2>Webhook Receipts</h2>{receipts.length===0?<p>No webhook security events recorded.</p>:receipts.map(r=><article key={r.id} style={{borderBottom:"1px solid #eee",padding:"8px 0"}}><strong>{r.topic}</strong><div><small>{r.processedAt.toISOString()} · correlation {r.correlationId}</small></div></article>)}</section>

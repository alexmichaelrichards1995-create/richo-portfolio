...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (intent === "execute") {
    ...
    await executeApprovedProductUpdate({
      ...
      actionId,
      expectedStateMatches: hashProductState(currentState) === row.expectedStateHash,
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const experiments = await listExperiments(session.shop);
  return { snapshot, decision, approvalQueue, productIntelligence, experiments, analyticsErrors: [...(data.shopifyqlQuery?.parseErrors ?? []), ...(data.sales?.parseErrors ?? [])] };
}
